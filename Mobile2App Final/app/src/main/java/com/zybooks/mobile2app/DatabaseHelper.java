...
// Initializes all variables to be used in databases
    ...
    // Method to search database using multiple query arguments in an array.
    ...
    // Export Method for events Enhancement 3
    // Restricted to exporting events from signed in user
    public Cursor getEventsForUser(int userId) {
        SQLiteDatabase db = this.getReadableDatabase();

        String query = "SELECT " + COL_EVENT_TITLE + ", " +
                COL_EVENT_DATE + ". " + COL_EVENT_TYPE +
                " FROM " + TABLE_EVENTS +
                " WHERE " + COL_EVENT_USER_ID + " = ?";
